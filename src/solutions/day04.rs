...
mod tests {
    ...
    fn test_passport_is_valid() {
        let s = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd\nbyr:1937 iyr:2017 cid:147 hgt:183cm";
        assert!(Passport::from_str(s).unwrap().is_valid());

        let s = "iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884\nhcl:#cfa07d byr:1929";
        assert!(!Passport::from_str(s).unwrap().is_valid());

        let s = "hcl:#ae17e1 iyr:2013\neyr:2024\necl:brn pid:760753108 byr:1931\nhgt:179cm";
        assert!(Passport::from_str(s).unwrap().is_valid());

        let s = "hcl:#cfa07d eyr:2025 pid:166559648\niyr:2011 ecl:brn hgt:59in";
        assert!(!Passport::from_str(s).unwrap().is_valid());
    }

    #[test]
    fn test_passport_is_strictly_valid() {
        let s = "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980 hcl:#623a2f";
        assert!(Passport::from_str(s).unwrap().is_strictly_valid());

        let s = "eyr:2029 ecl:blu cid:129 byr:1989 iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm";
        assert!(Passport::from_str(s).unwrap().is_strictly_valid());

        let s = "hcl:#888785 hgt:164cm byr:2001 iyr:2015 cid:88 pid:545766238 ecl:hzl eyr:2022";
        assert!(Passport::from_str(s).unwrap().is_strictly_valid());
    ...
    #[test]
    fn test_passport_is_not_strictly_valid() {
        let s = "eyr:1972 cid:100 hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926";
        assert!(!Passport::from_str(s).unwrap().is_strictly_valid());

        let s = "iyr:2019 hcl:#602927 eyr:1967 hgt:170cm ecl:grn pid:012533040 byr:1946";
        assert!(!Passport::from_str(s).unwrap().is_strictly_valid());

        let s = "hcl:dab227 iyr:2012 ecl:brn hgt:182cm pid:021572410 eyr:2020 byr:1992 cid:277";
        assert!(!Passport::from_str(s).unwrap().is_strictly_valid());

        let s = "hgt:59cm ecl:zzz eyr:2038 hcl:74454a iyr:2023 pid:3556412378 byr:2007";
        assert!(!Passport::from_str(s).unwrap().is_strictly_valid());
    }
}
